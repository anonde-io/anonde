...
(`Bash`, `Write`, `Edit`, …), and **warns** — or **blocks** — when sensitive
data is about to reach the model or leave your machine.
...
Pick one — they all end with `anonde-hook` on disk and the hooks wired up.

**A. Claude Code plugin (lowest friction).** Auto-registers the hooks and
fetches the binary for you — no `settings.json` edit, no Go toolchain:
...
See [`../../plugins/claude-code/`](../../plugins/claude-code/). The rest of
this page covers the **manual** path, for people who'd rather wire it by hand
or pin an exact binary.

**B. curl installer.** Downloads the prebuilt binary for your OS/arch:

```bash
curl -fsSL https://raw.githubusercontent.com/anonde-io/anonde/main/install.sh | sh
# installs anonde-hook into ~/.local/bin (override with ANONDE_INSTALL_DIR)
```

**C. Go toolchain.**

```bash
go install github.com/anonde-io/anonde/cmd/anonde-hook@latest   # into $(go env GOPATH)/bin
# or, from a checkout:
make build-hook                                                 # writes ./bin/anonde-hook
```

## Enable (manual path B/C)

The plugin (path A) does this for you. For the curl/Go installs, merge
[`settings.json`](./settings.json) into one of:

- `~/.claude/settings.json` — every project
- `.claude/settings.json` — this project (committable, shares the guard with your team)
...
If `anonde-hook` is not on Claude Code's PATH, use an absolute path or
`${CLAUDE_PROJECT_DIR}/bin/anonde-hook`.

Run `/hooks` inside Claude Code to confirm the hook is registered.

## Configure

All optional, set in your shell or in the hook's `command` (e.g.
`"command": "ANONDE_HOOK_MODE=block anonde-hook"`):
